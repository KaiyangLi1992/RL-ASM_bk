...
class MaskedGINDeepSigns(nn.Module):
    ...
    def forward(self, x, edge_index, batch_index):
        ...
        x = x.transpose(0, 1)  # N x K x In -> K x N x In1
        ...
        batched_num_nodes = torch.full((N,), N).to(edge_index.device)
        mask = torch.cat([torch.arange(K).unsqueeze(0) for _ in range(N)])
        mask = (mask.to(edge_index.device) < batched_num_nodes.unsqueeze(1)).bool()
