...
from torch_geometric.nn import  GATConv, GCNConv, TransformerConv, BatchNorm,GATv2Conv
...
def create_ourgmn_disentangled(dim_in, dim_out, q2t, t2q, gnn_subtype):
    ...
    if gnn_subtype == 'ours':
        ...
        gnn_t = GATv2Conv(dim_in, dim_out,dropout=0.1)
        gnn_q = GATv2Conv(dim_in, dim_out,dropout=0.1)
        ...
        if FLAGS.train_mode == 'RL':
            cfg.gt.attn_dropout = 0
    ...
    mlp_merge_t = MLP(*get_MLP_args([2 * dim_out, dim_out]))
    ...
    if FLAGS.train_mode == 'RL':
        ...
        dropout = 0.1
    gmn_inter = \
        OurGMNCustomInter(dim_in,dim_out,head_num = 4,dropout = dropout)
...
class OurGMNCustomWrapper(torch.nn.Module):
    ...
    def forward(self,pyg_data_q, pyg_data_t,
                  ...
                  only_run_inter=True):
        if only_run_inter:
            return self.gmn_inter(pyg_data_q, pyg_data_t,u2v_li)
        else:
...
class OurGMNCustomInter(torch.nn.Module):
    def __init__(self,dim_in,dim_out,head_num,dropout):
        super(OurGMNCustomInter, self).__init__()
        self.transformer_conv = TransformerConv(dim_in,dim_out,heads = head_num,dropout = dropout,concat=False)

     


    def forward(self, pyg_data_q, pyg_data_t, u2v_li):
        num_nodes_q = pyg_data_q.x.shape[0]

        edges = [[], []]
        for key, values in u2v_li.items():
            for v in values:
                edges[0].append(key)  # Append key to the first list
                edges[1].append(v+num_nodes_q)    # Append value to the second list
        for key, values in u2v_li.items():
            for v in values:
                edges[1].append(key)  # Append key to the first list
                edges[0].append(v+num_nodes_q)    # Append value to the second list

        # Convert lists to tensor
        
        edge_index = torch.tensor(edges, dtype=torch.long).to(FLAGS.device)
        concatenated_x = torch.cat((pyg_data_q.x, pyg_data_t.x), dim=0)
        concatenated_x = self.transformer_conv(concatenated_x,edge_index)

        Xq_merged = concatenated_x[:num_nodes_q,:]
        Xt_merged = concatenated_x[num_nodes_q:,:]
